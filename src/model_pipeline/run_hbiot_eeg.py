...
from lightning_datamodules.datamodules import (
    PredictionDataModule,
    SPikeDetector,
    infer_n_patches_per_channel,
)
...
def extract_paths(model_name):
    ...
    if os.path.basename(model_name) == "transformer.ckpt":
        config_path = "./utils_biot/config/hparams_eeg.yaml"
        checkpoint_path = model_name
        reference_channels_path = "./utils_biot/config/reference_channels_eeg.pkl"

    elif os.path.basename(model_name) == "hbiot.ckpt":
        config_path = "./utils_biot/config_hbiot/hparams_eeg.yaml"
        checkpoint_path = model_name
        reference_channels_path = "./utils_biot/config_hbiot/reference_channels_eeg.pkl"
...
def load_model(checkpoint_path, config):
    ...
    model = SPikeDetector.load_from_checkpoint(
        ...
        log_dir=None,
        n_patches_per_channel=infer_n_patches_per_channel(checkpoint_path),
    )
        ...
        for i in range(batch_size):
            ...
            if "window_times" in metadata:
                ...
                for j in range(n_windows):
                    ...
                    prob = float(sample_probs.reshape(-1)[j].item())  # Assuming binary classification
...
def filter_min_distance(results, min_distance=0.1):
    """
    Enforce a minimum temporal distance between consecutive predictions.

    Results are sorted by onset and greedily kept: a prediction is discarded
    when it is closer than ``min_distance`` seconds to the last kept one. This
    avoids reporting two consecutive (overlapping) windows.
    """
    kept = []
    last_onset = None
    for res in sorted(results, key=lambda r: r["onset"]):
        if last_onset is not None and res["onset"] - last_onset < min_distance:
            continue
        kept.append(res)
        last_onset = res["onset"]
    return kept
...
def test_model(
    ...
    results = filter_min_distance(results, min_distance=0.1)
